...
class EmailService {
  ...
  // Méthode pour forcer la réinitialisation du transporteur
  async reinitialize() {
    this.transporter = null;
  ...
  async sendNewDiligenceEmail(toEmail, userName, diligenceTitle, createdByName, diligenceId) {
    ...
    const subject = '📋 Nouvelle diligence assignée - Système de Diligence';
    ...
    const html = `
      ...
      <body>
        <div class="container">
          ...
          <div class="content">
            <h2>Bonjour ${userName},</h2>
            <p>Une nouvelle diligence vous a été assignée sur notre plateforme.</p>
            
            <div class="diligence-info">
              <h3 style="margin-top: 0; color: #667eea;">${diligenceTitle}</h3>
              <p><strong>Assignée par :</strong> ${createdByName}</p>
              <p><strong>Date d'envoi :</strong> ${new Date().toLocaleDateString('fr-FR')}</p>
            </div>
            
            <p>Vous pouvez consulter les détails de cette diligence en cliquant sur le bouton ci-dessous :</p>
            ...
            <p><strong>Action requise :</strong> Veuillez prendre connaissance de cette diligence et mettre à jour son statut en fonction de l'avancement.</p>
          </div>
          <div class="footer">
            <p>Cet email a été envoyé automatiquement, merci de ne pas y répondre.</p>
            <p>© 2025 Système de Diligence. Tous droits réservés.</p>
